...
class UserViewSet(
    ...
    """
    This viewset provides `detail` and `create` actions. The `list` action is
    restricted to superusers to avoid disclosing every username in the database.
    """
    ...
    def create(self, request, *args, **kwargs):
        """
        Create a new user account. Restricted to superusers.
        Expects JSON payload with 'username', 'password', and optional 'email',
        'first_name', 'last_name'.
        """
        if not request.user.is_superuser:
            raise exceptions.PermissionDenied(
                detail="Only superusers can create user accounts."
            )
